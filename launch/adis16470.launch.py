...
def generate_launch_description():
    ...
    bias_node_run=Node(
        package="adrd2121_imu",
        name="bias_estimate_node_ros2",
        executable="bias_estimate_node_ros2",
        parameters=[
            {"gyro_std_thresh":0.90},
            {"accl_std_thresh":0.90}],
        output="screen",
        emulate_tty=True,
        arguments=["--ros-args", "--log-level", 
            PythonExpression(expression=["'","bias_estimate_node_ros2:=",log_level,"'"])]
    )
